...
- 当前分支为 `main`，识别、图传叠加和图传队列仍共用同一张 `320x240 YUV422` 相机帧：`yahboom_camera.c` 取帧后调用 `yahboom_detection_process()`，再叠加 FPS，最后投递 HTTP 图传队列；未引入独立 JPEG 图传任务。
...
- 2026-08-07 用户要求识别标记改为红色固定方框并删除中心十字；`yahboom_detection.c` 已将方框改为 YUV `{76,85,255}`，保留固定 `12x12` 尺寸，不再绘制十字。识别算法和 MSP 输出不变。
